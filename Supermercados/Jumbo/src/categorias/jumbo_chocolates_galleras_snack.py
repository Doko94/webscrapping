...
OUTPUT_DIR = Path(os.getenv("SCRAPER_OUTPUT_DIR", "output/chocolates_galletas_snacks"))
...
OUT_PREFIX = "jumbo_chocolates_galletas_y_snacks"

# -------------------------
# Subcategorías (manual) según menú
# -------------------------
SUBCATS = [
    # CHOCOLATES
    {"name": "Chocolates en Barra", "url": f"{BASE}/chocolates-galletas-y-snacks/chocolates/chocolates-en-barra"},
    {"name": "Bombones y Trufas", "url": f"{BASE}/chocolates-galletas-y-snacks/chocolates/bombones-y-trufas"},
    {"name": "Chocolates y Tentaciones", "url": f"{BASE}/chocolates-galletas-y-snacks/chocolates/chocolates-y-tentaciones"},
    {"name": "Chocolates sin Azúcar", "url": f"{BASE}/chocolates-galletas-y-snacks/chocolates/chocolates-sin-azucar"},
    {"name": "Chocolates (Mostrar todo)", "url": f"{BASE}/chocolates-galletas-y-snacks/chocolates"},

    # GALLETAS DULCES
    {"name": "Galletas Rellenas y Bañadas", "url": f"{BASE}/chocolates-galletas-y-snacks/galletas-dulces/galletas-rellenas-y-banadas"},
    {"name": "Galletas Mini", "url": f"{BASE}/chocolates-galletas-y-snacks/galletas-dulces/galletas-mini"},
    {"name": "Galletas Tradicionales", "url": f"{BASE}/chocolates-galletas-y-snacks/galletas-dulces/galletas-tradicionales"},
    {"name": "Galletas Obleas y Champagne", "url": f"{BASE}/chocolates-galletas-y-snacks/galletas-dulces/galletas-obleas-y-champagne"},
    {"name": "Galletas Dulces (Mostrar todo)", "url": f"{BASE}/chocolates-galletas-y-snacks/galletas-dulces"},

    # DULCES
    {"name": "Gomitas y Chicles", "url": f"{BASE}/chocolates-galletas-y-snacks/dulces/gomitas-y-chicles"},
    {"name": "Masticables y Caramelos", "url": f"{BASE}/chocolates-galletas-y-snacks/dulces/calugas-masticables-y-caramelos"},
    {"name": "Malvas, Marshmallows y Merenguitos", "url": f"{BASE}/chocolates-galletas-y-snacks/dulces/malvas-marshmallows-y-merenguitos"},
    {"name": "Alfajores, Pasteles y Queques", "url": f"{BASE}/chocolates-galletas-y-snacks/dulces/alfajores-pasteles-y-queques"},
    {"name": "Dulces (Mostrar todo)", "url": f"{BASE}/chocolates-galletas-y-snacks/dulces"},

    # SNACKS
    {"name": "Papas Fritas", "url": f"{BASE}/chocolates-galletas-y-snacks/snacks/papas-fritas"},
    {"name": "Snacks Vegetales", "url": f"{BASE}/chocolates-galletas-y-snacks/snacks/snacks-vegetales"},
    {"name": "Maní y Frutos Secos", "url": f"{BASE}/chocolates-galletas-y-snacks/snacks/mani-y-frutos-secos"},
    {"name": "Ramitas, Suflés y Otros Snacks", "url": f"{BASE}/chocolates-galletas-y-snacks/snacks/ramitas-sufles-y-otros-snacks"},
    {"name": "Snacks (Mostrar todo)", "url": f"{BASE}/chocolates-galletas-y-snacks/snacks"},

    # GALLETAS SALADAS
    {"name": "Galletas de Cóctel", "url": f"{BASE}/chocolates-galletas-y-snacks/galletas-saladas/galletas-de-coctel"},
    {"name": "Galletas de Arroz y Quinoa", "url": f"{BASE}/chocolates-galletas-y-snacks/galletas-saladas/galletas-de-arroz-y-quinoa"},
    {"name": "Galletas de Soda y Agua", "url": f"{BASE}/chocolates-galletas-y-snacks/galletas-saladas/galletas-de-soda-y-agua"},
    {"name": "Galletas Integrales y Salvado", "url": f"{BASE}/chocolates-galletas-y-snacks/galletas-saladas/galletas-integrales-y-salvado"},
    {"name": "Galletas Saladas (Mostrar todo)", "url": f"{BASE}/chocolates-galletas-y-snacks/galletas-saladas"},

    # PASTAS PARA CÓCTEL Y UNTABLES
    {"name": "Pastas Untables", "url": f"{BASE}/chocolates-galletas-y-snacks/pastas-para-coctel-y-untables/pastas-untables"},
    {"name": "Salsas y Dip Untables", "url": f"{BASE}/chocolates-galletas-y-snacks/pastas-para-coctel-y-untables/salsas-y-dip-untables"},
    {"name": "Hummus", "url": f"{BASE}/chocolates-galletas-y-snacks/pastas-para-coctel-y-untables/hummus"},
    {"name": "Pastas para Cóctel y Untables (Mostrar todo)", "url": f"{BASE}/chocolates-galletas-y-snacks/pastas-para-coctel-y-untables"},
]
...
def is_bff_plp_url(url: str) -> bool:
    u = url.lower()
    return "bff.jumbo.cl/catalog/plp" in u

def sanitize_headers(h: Dict[str, str]) -> Dict[str, str]:
    # API request falla si envías headers “prohibidos” o inconsistentes
    drop = {
    ...
    for k, v in h.items():
        if k.lower() in drop:
            continue
        out[k] = v
    # Asegurar content-type para POST JSON
...
def mutate_pagination(payload: dict, page_num: int) -> dict:
    """
    Jumbo a veces pagina por:
      - page / currentPage
      - from + size
      - offset + limit
      - pagination: { page, size } ...
    Esta función intenta cubrir todo.
    """
    p = json.loads(json.dumps(payload))  # deep copy

    # Caso 1: page / currentPage directo
    ...
    # Fallback: intenta setear page aunque no exista
    p["page"] = page_num
    return p

async def capture_seed_bff_request(page: Page, url: str, timeout_ms: int = 120000) -> Tuple[Optional[dict], Optional[dict]]:
    """
    Navega una vez al PLP y captura el response del BFF (/catalog/plp),
    devolviendo:
      - seed: { url, method, headers, post_data_json }
      - data_json: el JSON del response
    """
...
async def fetch_bff_page(context, seed: dict, page_num: int) -> Tuple[str, Optional[dict]]:
    """
    Repite el request BFF (sin UI) cambiando la paginación.
    """
    url = seed["url"]
...
async def scrape_subcategory(context, page: Page, name: str, url: str, max_pages: int) -> List[dict]:
    ...
    # Reintentos por si Jumbo está lento
    seed = None
    data0 = None
    ...
    # Página 1 (desde el response capturado)
    src_url = seed["url"]
    ...
    # Páginas 2..N vía API (sin clicks)
    prev_count = len(all_by_sku)
    for page_num in range(2, max_pages + 1):
        # pequeño “respirito” para no gatillar rate-limit
        ...
        # corte si ya no crece (evita loops raros)
...
async def main():
    async with async_playwright() as p:
        ...
        # ✅ UNA SOLA PESTAÑA PARA TODO (como en carnes)
