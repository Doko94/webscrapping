...
OUTPUT_DIR = Path(os.getenv("SCRAPER_OUTPUT_DIR", "output/bebidas_y_snacks"))
BASE = "https://super.lider.cl"

HEADLESS = os.getenv("LIDER_HEADLESS", "1").strip().lower() not in {"0", "false", "no"}
DEBUG = True
OUT_PREFIX = "lider_bebidas_y_snacks"

CATEGORY_NAME = "Bebidas y Snacks"
...
async def discover_subcategories(browser: Browser) -> List[dict]:
    ...
    try:
        ...
        if category_el is None:
            raise RuntimeError("No encontré 'Bebidas y Snacks' en el menú lateral")
        ...
        try:
            ...
            print(f"[DEBUG] Click en Bebidas y Snacks falló: {e}")
        ...
        for item in links:
            ...
            if "/browse/bebidas-y-snacks/" not in full_url_low:
                continue
...
async def main():
    async with async_playwright() as p:
        ...
        if not subcats:
            raise RuntimeError("No se pudieron descubrir subcategorías para Bebidas y Snacks")
