...
OUTPUT_DIR = Path(os.getenv("SCRAPER_OUTPUT_DIR", "output/panaderia_y_pasteleria"))
BASE = "https://super.lider.cl"

HEADLESS = True
DEBUG = True
OUT_PREFIX = "lider_panaderia_y_pasteleria"

CATEGORY_NAME = "Panadería y Pastelería"
...
async def discover_subcategories(browser: Browser) -> List[dict]:
    ...
    try:
        ...
        if category_el is None:
            raise RuntimeError("No encontré 'Panadería y Pastelería' en el menú lateral")
        ...
        try:
            ...
            print(f"[DEBUG] Click en Panadería y Pastelería falló: {e}")
        ...
        for item in links:
            ...
            if "/browse/panaderia-y-pasteleria/" not in full_url_low:
                continue
...
async def main():
    async with async_playwright() as p:
        ...
        if not subcats:
            raise RuntimeError("No se pudieron descubrir subcategorías para Panadería y Pastelería")
