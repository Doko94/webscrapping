...
OUTPUT_DIR = Path(os.getenv("SCRAPER_OUTPUT_DIR", "output/limpieza_aseo"))
BASE = "https://super.lider.cl"

HEADLESS = os.getenv("LIDER_HEADLESS", "1").strip().lower() not in {"0", "false", "no"}
DEBUG = True
OUT_PREFIX = "lider_limpieza_aseo"

HOME_URL = BASE
CATEGORY_NAME = "Limpieza y Aseo"
CATEGORY_LANDING = f"{BASE}/browse/limpieza-y-aseo"
...
async def random_pause(a=0.3, b=1.0):
...
async def discover_subcategories(browser: Browser) -> List[dict]:
    ...
    async def safe_hover(locator, timeout=3000):
        try:
    ...
    try:
        ...
        if await is_blocked(page):
            if not await wait_for_manual_unblock(page, "Home de Lider"):
        ...
        category_selectors = [
            'text="Limpieza y Aseo"',
            'a:has-text("Limpieza y Aseo")',
            'button:has-text("Limpieza y Aseo")',
            '[role="menuitem"]:has-text("Limpieza y Aseo")',
        ]
        ...
        for sel in category_selectors:
            try:
                ...
                if (
                    "detergentes" in body_text
                    and "papeles" in body_text
                    and "desinfección" in body_text
                ):
        ...
        if not category_activated:
            raise RuntimeError("No se pudo desplegar el mega menú de Limpieza y Aseo")
        ...
        data = await page.evaluate(
            ...
            () => {
                ...
                const allNodes = Array.from(
                    document.querySelectorAll("h1, h2, h3, h4, h5, h6, strong, p, span, div, a")
                );
...
async def get_dom_signature(page: Page) -> str:
    try:
        ...
        sig = "||".join(sorted((x.get("sku") or "") for x in items[:24]))
        return sig
    except Exception:
        return ""
...
# =========================
# Scroll dinámico optimizado
# =========================
async def progressive_scroll(page: Page, max_rounds: int = SCROLL_MAX_ROUNDS, stable_rounds_needed: int = SCROLL_STABLE_ROUNDS):
    last_height = -1
    last_anchor_count = -1
    stable_rounds = 0
    ...
    for i in range(max_rounds):
        try:
            anchor_count_before = await page.locator('a[href*="/ip/"], a[href*="/p/"]').count()
        except Exception:
        ...
        for _ in range(2):
            try:
                await page.mouse.wheel(0, random.randint(1800, 3000))
            except Exception:
                pass
        ...
        try:
            anchor_count_after = await page.locator('a[href*="/ip/"], a[href*="/p/"]').count()
        except Exception:
        ...
        if DEBUG:
            print(
                f"      scroll round {i+1}: "
                f"anchors_before={anchor_count_before}, anchors_after={anchor_count_after}, "
                f"height_before={height_before}, height_after={height_after}"
            )

        if anchor_count_after == last_anchor_count and height_after == last_height:
            stable_rounds += 1
        else:
            stable_rounds = 0

        last_anchor_count = anchor_count_after
        last_height = height_after

        if stable_rounds >= stable_rounds_needed:
            if DEBUG:
                print(f"      DOM estabilizado tras {i+1} rondas de scroll")
            break
...
async def scrape_subcategory(browser: Browser, group: str, name: str, url: str) -> Tuple[List[dict], bool]:
    ...
    try:
        ...
        for page_num in range(1, MAX_PAGES + 1):
            if DEBUG:
                print(f"    page {page_num}: procesando DOM [{group} > {name}]")

            await progressive_scroll(page)

            rows_1 = await extract_products_from_dom(page)
            ...
            rows_2 = await extract_products_from_dom(page)

            page_rows_map = {}
            for r in rows_1 + rows_2:
                sku = r.get("sku")
                if not sku:
                    continue
                if sku not in page_rows_map or row_quality_score(r) > row_quality_score(page_rows_map[sku]):
                    page_rows_map[sku] = r

            rows = list(page_rows_map.values())
            ...
            sample_skus = [r.get("sku") for r in rows[:8]]
...
# Worker paralelo
# =========================
async def scrape_worker(browser: Browser, semaphore: asyncio.Semaphore, idx: int, total: int, sc: dict):
    async with semaphore:
        print(f"\n[{idx}/{total}] Scrape: [{sc['group']}] {sc['name']}")
...
async def main():
    async with async_playwright() as p:
        ...
        if not subcats:
            raise RuntimeError("No se pudieron descubrir subcategorías para Limpieza y Aseo")

        semaphore = asyncio.Semaphore(MAX_CONCURRENT_SUBCATS)

        tasks = [
            scrape_worker(browser, semaphore, idx, len(subcats), sc)
        ...
        print("\n==============================")
        print("Total subcategorías:", len(subcats))
        print("Subcategorías bloqueadas:", blocked_count)
        print("Total filas finales:", len(out_rows))
