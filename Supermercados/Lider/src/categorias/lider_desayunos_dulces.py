...
HEADLESS = True
DEBUG = True
OUT_PREFIX = "lider_desayunos_y_dulces"

CATEGORY_NAME = "Desayunos y Dulces"
...
async def is_blocked(page: Page) -> bool:
    ...
    try:
        ...
        if "robot or human" in body:
            return True
        if "activate and hold the button to confirm that you’re human" in body:
            return True
    except Exception:
        pass
...
def parse_product(p: dict) -> Optional[dict]:
    ...
    return {
        "sku": str(sku).strip(),
        "name": str(name).strip(),
        "brand": str(deep_find_first(p, ["brand", "brandName", "Brand", "brandText"]) or "").strip(),
        "price": price,
...
# Modo fijo
# =========================
def get_subcategories_fixed() -> List[dict]:
    """
    Pega aquí las URLs FINALES con IDs cuando las tengas.
    Mientras estén vacías, el script intentará descubrir desde el menú.
    """
    subcats = [
        # Café Té y Hierbas
        {"group": "Café Té y Hierbas", "name": "Café Instantáneo", "url": ""},
        {"group": "Café Té y Hierbas", "name": "Café en Grano y Molido", "url": ""},
        {"group": "Café Té y Hierbas", "name": "Cápsulas de Café", "url": ""},
        {"group": "Café Té y Hierbas", "name": "Café Descafeinado", "url": ""},
        {"group": "Café Té y Hierbas", "name": "Café Saborizados y Cremas", "url": ""},
        {"group": "Café Té y Hierbas", "name": "Té Ceylan", "url": ""},
        {"group": "Café Té y Hierbas", "name": "Té Especiales y Sabores", "url": ""},
        {"group": "Café Té y Hierbas", "name": "Hierbas y Yerba Mate", "url": ""},

        # Cereales
        {"group": "Cereales", "name": "Barras de Cereal", "url": ""},
        {"group": "Cereales", "name": "Cereales", "url": ""},
        {"group": "Cereales", "name": "Cereal Trigo Inflado", "url": ""},
        {"group": "Cereales", "name": "Colaciones Escolares", "url": ""},
        {"group": "Cereales", "name": "Avenas", "url": ""},

        # Galletas y Colaciones Dulces
        {"group": "Galletas y Colaciones Dulces", "name": "Rellenas Bañadas", "url": ""},
        {"group": "Galletas y Colaciones Dulces", "name": "Chips Chocolate", "url": ""},
        {"group": "Galletas y Colaciones Dulces", "name": "Clásicas Obleas y Champaña", "url": ""},
        {"group": "Galletas y Colaciones Dulces", "name": "Saludables", "url": ""},
        {"group": "Galletas y Colaciones Dulces", "name": "Galletas Colación", "url": ""},
        {"group": "Galletas y Colaciones Dulces", "name": "Galletones y Alfajores", "url": ""},
        {"group": "Galletas y Colaciones Dulces", "name": "Quequitos y Bizcochos", "url": ""},

        # Dulces Mermeladas y Manjar
        {"group": "Dulces Mermeladas y Manjar", "name": "Azúcar", "url": ""},
        {"group": "Dulces Mermeladas y Manjar", "name": "Endulzante", "url": ""},
        {"group": "Dulces Mermeladas y Manjar", "name": "Manjar y Dulce de Leche", "url": ""},
        {"group": "Dulces Mermeladas y Manjar", "name": "Mermeladas", "url": ""},
        {"group": "Dulces Mermeladas y Manjar", "name": "Miel y Dulces", "url": ""},

        # Chocolates y Candy
        {"group": "Chocolates y Candy", "name": "Caramelos Duros", "url": ""},
        {"group": "Chocolates y Candy", "name": "Calugas y Masticables", "url": ""},
        {"group": "Chocolates y Candy", "name": "Gomitas", "url": ""},
        {"group": "Chocolates y Candy", "name": "MarshMallows", "url": ""},
        {"group": "Chocolates y Candy", "name": "Chicles Mentitas", "url": ""},
        {"group": "Chocolates y Candy", "name": "Artesanales", "url": ""},

        # Postres para Preparar
        {"group": "Postres para Preparar", "name": "Gelatina en Polvo", "url": ""},
        {"group": "Postres para Preparar", "name": "Flan y Budín", "url": ""},
    ]
    return [x for x in subcats if x["url"].strip()]


# =========================
# Descubrir subcategorías desde menú
# =========================
async def open_categories_menu(page: Page) -> bool:
    selectors = [
    ...
    for sel in selectors:
        try:
            ...
            for _ in range(5):
                ...
                if "Todas las categorías" in body_text and "Marcas Propias" in body_text:
                    return True
...
async def reveal_left_category(page: Page, category_name: str) -> bool:
    selectors = [
...
async def discover_subcategories(browser: Browser) -> List[dict]:
    ...
    try:
        ...
        opened = await open_categories_menu(page)
        if not opened:
            raise RuntimeError("No pude abrir el menú de Categorías")

        found = await reveal_left_category(page, CATEGORY_NAME)
        if not found:
            raise RuntimeError("No encontré 'Desayunos y Dulces' en el menú lateral")
        ...
        category_el = None
        for sel in category_candidates:
            try:
                loc = page.locator(sel).first
                if await loc.count() > 0 and await loc.is_visible():
                    if DEBUG:
        ...
        try:
            await category_el.hover(timeout=3000)
        ...
        try:
            await category_el.click(timeout=3000)
            await page.wait_for_timeout(1800)
        except Exception as e:
            if DEBUG:
                print(f"[DEBUG] Click en Desayunos y Dulces falló: {e}")
        ...
        for item in links:
            ...
            if "/browse/desayunos-y-dulces/" not in full_url_low:
                continue
...
async def main():
    async with async_playwright() as p:
        ...
        fixed_subcats = get_subcategories_fixed()
        if fixed_subcats:
