...
HOME_URL = BASE
CATEGORY_NAME = "Colaciones"
CATEGORY_LANDING = f"{BASE}/browse/colaciones"
...
async def discover_subcategories(browser: Browser) -> List[dict]:
    ...
    try:
        ...
        # 2) Activar Colaciones en la columna izquierda
        category_activated = False
        category_selectors = [
            'text="Colaciones"',
            'a:has-text("Colaciones")',
            'button:has-text("Colaciones")',
            '[role="menuitem"]:has-text("Colaciones")',
        ]
        ...
        for sel in category_selectors:
            try:
                ...
                if (
                    "fruta colación" in body_text
                    and "barritas y cereales" in body_text
                    and "leches colación" in body_text
                ):
        ...
        if not category_activated:
            raise RuntimeError("No se pudo desplegar el mega menú de Colaciones")
...
async def main():
    async with async_playwright() as p:
        ...
        if not subcats:
            raise RuntimeError("No se pudieron descubrir subcategorías para Colaciones")
