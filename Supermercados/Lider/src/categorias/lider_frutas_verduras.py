...
OUTPUT_DIR = Path(os.getenv("SCRAPER_OUTPUT_DIR", "output/frutas_y_verduras"))
BASE = "https://super.lider.cl"

HEADLESS = True
DEBUG = True
OUT_PREFIX = "lider_frutas_y_verduras"
...
async def discover_subcategories(browser: Browser) -> List[dict]:
    ...
    try:
        ...
        if category_el is None:
            raise RuntimeError("No encontré 'Frutas y Verduras' en el menú lateral")
        ...
        try:
            ...
            print(f"[DEBUG] Click en Frutas y Verduras falló: {e}")
        ...
        for item in links:
            ...
            if "/browse/frutas-y-verduras/" not in full_url_low:
                continue
...
async def main():
    async with async_playwright() as p:
        ...
        if not subcats:
            raise RuntimeError("No se pudieron descubrir subcategorías para Frutas y Verduras")
