...
DEBUG = True
OUT_PREFIX = "lider_mascotas"

HOME_URL = BASE
CATEGORY_NAME = "Mascotas"
CATEGORY_LANDING = f"{BASE}/browse/mascotas"
...
async def discover_subcategories(browser: Browser) -> List[dict]:
    ...
    try:
        ...
        # 2) Activar Mascotas en la columna izquierda
        category_activated = False
        category_selectors = [
            'text="Mascotas"',
            'a:has-text("Mascotas")',
            'button:has-text("Mascotas")',
            '[role="menuitem"]:has-text("Mascotas")',
        ]
        ...
        for sel in category_selectors:
            try:
                ...
                if (
                    "perro" in body_text
                    and "gato" in body_text
                    and "higiene y bienestar" in body_text
                ):
        ...
        if not category_activated:
            raise RuntimeError("No se pudo desplegar el mega menú de Mascotas")
...
async def main():
    async with async_playwright() as p:
        ...
        if not subcats:
            raise RuntimeError("No se pudieron descubrir subcategorías para Mascotas")
