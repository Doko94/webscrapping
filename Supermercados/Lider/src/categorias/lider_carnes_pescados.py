...
DEBUG = True
MAX_PAGES = 60
OUT_PREFIX = "lider_carnes_y_pescados"

CATEGORY_NAME = "Carnes y Pescados"
CATEGORY_LANDING = f"{BASE}/content/carnes-y-pescados/21856785"

BLOCK_PATTERNS = [
    # dejamos solo trackers más evidentes
...
async def discover_subcategories(browser: Browser) -> List[dict]:
    ...
    try:
        ...
        carnes_candidates = [
            ...
            "a:has-text('Carnes y Pescados')",
            "div:has-text('Carnes y Pescados')",
            "li:has-text('Carnes y Pescados')",
            "span:has-text('Carnes y Pescados')",
        ]

        carnes_el = None
        for sel in carnes_candidates:
            try:
                loc = page.locator(sel).first
                if await loc.count() > 0 and await loc.is_visible():
                    print(f"[DEBUG] Encontrado 'Carnes y Pescados' con selector: {sel}")
                    carnes_el = loc
                    break
            except Exception as e:
                print(f"[DEBUG] Falló búsqueda selector {sel}: {e}")

        if carnes_el is None:
            raise RuntimeError("No encontré 'Carnes y Pescados' en el menú lateral")

        try:
            await carnes_el.hover(timeout=3000)
            await page.wait_for_timeout(1200)
        except Exception as e:
            print(f"[DEBUG] Hover falló: {e}")

        try:
            await carnes_el.click(timeout=3000)
            await page.wait_for_timeout(2000)
        except Exception as e:
            print(f"[DEBUG] Click en Carnes y Pescados falló: {e}")
        ...
        for item in links:
            ...
            if group not in TARGET_GROUPS or not href:
                continue

            full_url = urljoin(BASE, href)
            full_url_low = full_url.lower()

            if "/browse/carnes-y-pescados/" not in full_url_low:
                continue

            if "contentzone" in full_url_low or "co_ty=" in full_url_low or "co_nm=" in full_url_low:
                continue

            if name.lower() in {"revisar todo", "ir a la categoría", "¡descubre más!"}:
                continue
...
async def main():
    async with async_playwright() as p:
        ...
        if not subcats:
            raise RuntimeError("No se pudieron descubrir subcategorías para Carnes y Pescados")
