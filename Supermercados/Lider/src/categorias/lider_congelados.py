...
async def discover_subcategories(browser: Browser) -> List[dict]:
    ...
    try:
        ...
        if not found:
            raise RuntimeError("No encontré 'Congelados' en el menú lateral")
        ...
        try:
            ...
            if DEBUG:
                print(f"[DEBUG] Click en Congelados falló: {e}")
        ...
        for item in links:
            ...
            if "/browse/congelados/" not in full_url_low:
                continue
...
async def main():
    async with async_playwright() as p:
        ...
        if not subcats:
            raise RuntimeError("No se pudieron descubrir subcategorías para Congelados")
