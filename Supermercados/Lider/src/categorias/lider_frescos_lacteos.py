...
CATEGORY_NAME = "Frescos y Lácteos"
...
async def discover_subcategories(browser: Browser) -> List[dict]:
    ...
    try:
        ...
        if category_el is None:
            raise RuntimeError("No encontré 'Frescos y Lácteos' en el menú lateral")
        ...
        try:
            ...
            print(f"[DEBUG] Click en Frescos y Lácteos falló: {e}")
        ...
        for item in links:
            ...
            if "/browse/frescos-y-lacteos/" not in full_url_low:
                continue
...
async def main():
    async with async_playwright() as p:
        ...
        if not subcats:
            raise RuntimeError("No se pudieron descubrir subcategorías para Frescos y Lácteos")
